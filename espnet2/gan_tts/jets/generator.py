...
class JETSGenerator(torch.nn.Module):
    ...
    def __init__(
        ...
        generator_resblock_dilations: List[List[int]] = [[1, 3, 5], [1, 3, 5], [1, 3, 5]],
        ...
        Args:
            ...
            generator_resblock_dilations (List[List[int]]): List of list of dilations for residual
                blocks.
            generator_use_additional_convs (bool): Whether to use additional conv layers in
                residual blocks.
            ...
            generator_nonlinear_activation_params (Dict[str, Any]): Hyperparameters for activation
                function.
            generator_use_weight_norm (bool): Whether to use weight norm. If set to true, it will
    ...
    def inference(
        ...
        if use_teacher_forcing:
            ...
            p_outs = average_by_duration(d_outs, pitch.squeeze(-1), text_lengths, feats_lengths).unsqueeze(-1)
            e_outs = average_by_duration(d_outs, energy.squeeze(-1), text_lengths, feats_lengths).unsqueeze(-1)
        else:
            # forward duration predictor and variance predictors
            p_outs = self.pitch_predictor(hs, h_masks.unsqueeze(-1))
            e_outs = self.energy_predictor(hs, h_masks.unsqueeze(-1))
